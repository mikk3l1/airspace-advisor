...
def calc_mission_collision(mission_waypoints: list, air_traffic_dict: dict) -> list:
    ...
    for aircraft in air_traffic_dict:
        aircraft_new_coordinates_list = air_traffic_dict[aircraft]['new_coordinates'].get('60')
        ...
        # if any(waypoint_aircraft_distance_dict) < 0.6096 and waypoint[2]-aircraft_new_coordinates_list[2] < 200:
        # print(aircraft,' is this far from waypoints', waypoint_aircraft_distance_dict, '\n')
        if any(waypoint_aircraft_distance_dict) < 3 and waypoint[2]-aircraft_new_coordinates_list[2] < 2000:
            # mission_collision_str += f'Carefull! {aircraft} will collide with your mission within the next 60 seconds!'
            return ['warn',f'Carefull! {aircraft} will collide with your mission within the next 60 seconds!']
        # else:
...
def calc_drone_aircraft_collision(drone_dict: dict, air_traffic_dict: dict) -> list:
    ...
    drone_new_coordinates_lat_long = drone_dict['new_coordinates'].get('60')
    for aircraft in air_traffic_dict:
        # print(aircraft)
        # print(air_traffic_dict[aircraft])
        aircraft_new_coordinates_list_lat_long = air_traffic_dict[aircraft]['new_coordinates'].get('60')
        ...
        # if distance_between_drone_aircraft < 0.6096 and altitude_between_drone_aircraft < 200:
        # print(distance_between_drone_aircraft, f'distance between drone and {aircraft}')
        # print(altitude_between_drone_aircraft, f'altitude between drone and {aircraft}\n')
        if distance_between_drone_aircraft < 1 and altitude_between_drone_aircraft < 2000:
